...
import { Activity, Plus, MapPin, Calendar, CheckSquare } from "lucide-react"
...
export default async function AdminCulturalPage() {
  const events = await prisma.culturalEvent.findMany({
    ...
    include: {
      _count: {
        select: { assignments: true, reports: true }
      }
    }
  })
  ...
  return (
    <div className="p-8 md:p-12 max-w-6xl mx-auto space-y-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
        <div>
          <h1 className="text-4xl font-display font-black text-brand tracking-tight">Cultural Events</h1>
          <p className="text-muted font-medium mt-2">Manage festivals, club gatherings, and volunteer assignments.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-surface-alt text-brand">
                <th className="p-6 font-bold uppercase tracking-widest text-xs">Event Details</th>
                <th className="p-6 font-bold uppercase tracking-widest text-xs">Schedule & Venue</th>
                <th className="p-6 font-bold uppercase tracking-widest text-xs">Duty Assignments</th>
              ...
              {events.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-12 text-center text-muted font-medium">
                    No cultural events found. Click "Create Cultural Event" to add one!
                  </td>
                </tr>
              ) : events.map((event) => (
                <tr key={event.id} className="hover:bg-brand/5 transition-colors group">
                  <td className="p-6">
                    ...
                    <p className="text-sm font-medium text-muted mt-1 truncate max-w-[250px]">{event.description}</p>
                  </td>
                  ...
                  <td className="p-6">
                    <div className="flex items-center gap-2 text-sm font-bold text-ink/70">
                      ...
                      {event._count.assignments} Volunteers Assigned
                    </div>
                  </td>
                  <td className="p-6">
                    {event.status === 'LIVE' && (
                  ...
                  <td className="p-6 text-right space-x-4">
                    <button className="text-brand font-bold text-sm hover:underline">Assignments</button>
