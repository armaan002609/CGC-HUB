...
export function LiveMatchCard({ match }: { match: any }) {
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-3xl border border-black/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 flex flex-col h-full">
      ...
      <div className="bg-surface-alt/50 px-4 sm:px-8 py-3 sm:py-4 border-t border-black/5 flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] sm:text-xs font-medium text-muted">
        ...
        <div className="flex items-center gap-1.5 w-full sm:w-auto justify-center sm:justify-start">
          ...
          Started {new Date(match.schedule).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
        </div>
      </div>
